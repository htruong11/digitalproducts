...
IMP-001 → IMP-003 cover *what* we're selling (caregiver printable PDFs on Etsy)
and *how* the agent system will run. This is the **owner's checklist**: the
...
is a ranked, do-this-now runbook. Everything is sorted by priority. Each task is
...
- 🟢 **AGENT-READY** — a coding agent (Claude) can do this now. Listed so you know
    to *delegate* it, not do it yourself.
...
- The product is decided — IMP-001's working title is the **"Family Caregiver
    Command Kit"**. You still need an **Etsy-legal, memorable, non-trademarked
...
- *Agent can help:* ask Claude to brainstorm 20 names and pre-check Etsy/
...
## Priority 2 — Stand Up the Tech Foundation (mix, ~45 min of *your* time)
...
- *Optional, later:* Gemini API key (free tier) for the research scout, and a
...
- The **products are PDFs**, so no AI image generator is required to ship. You
    *do* need hero/preview **mockups** (the printable shown on a clipboard/in a
...
- 🟢 Agent produces the first product — the **~10–14 page Family Caregiver
    Command Kit** (IMP-001 MVP spec: Start Here, emergency sheet, medication
    tracker, appointment tracker, handoff log, etc.) as a print-ready,
...
> Command Kit from the IMP-001 MVP spec as a print-ready PDF with the disclaimer,
...
*Owner's runbook. Do the red items; delegate the rest.*
